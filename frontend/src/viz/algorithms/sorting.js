export const SORTING_ALGORITHMS = {
  bubble_sort: {
    ...
    // Genera la secuencia de estados visuales para el ordenamiento de burbuja
    generateSteps({ array }) {
      const steps = [];
      const a = [...array];
      const n = a.length;
  ...
  quick_sort: {
    ...
    // Genera la secuencia de estados visuales para el ordenamiento rápido
    generateSteps({ array }) {
      const steps = [];
      const a = [...array];

      // Divide el subarreglo tomando el último elemento como pivote y reubica los menores a su izquierda
      function partition(low, high) {
        ...
        for (let j = low; j < high; j++) {
          steps.push({
            array: [...a], comparing: [j, high], sorted: [],
            pivot: high, phase: 'compare',
          ...
          if (a[j] <= pivot) {
            ...
            if (i !== j) {
              steps.push({
                array: [...a], comparing: [i, j], sorted: [],
                pivot: high, phase: 'swap',
        ...
        steps.push({
          array: [...a], comparing: [], sorted: [i + 1],
          pivot: null, phase: 'place',
      ...
      // Función recursiva que aplica Quick Sort a particiones sucesivas
      ...
      steps.push({
        ...
        pivot: null, phase: 'done',
        description: '¡Arreglo ordenado con Quick Sort! ✓'
      });
  ...
  merge_sort: {
    ...
    // Genera la secuencia de estados visuales para el ordenamiento por mezcla
    generateSteps({ array }) {
      const steps = [];
      const a = [...array];
      ...
      function merge(low, mid, high) {
        ...
        while (i < left.length && j < right.length) {
          steps.push({
            array: [...a], comparing: [low + i, mid + 1 + j], sorted: [],
      ...
      // Función recursiva que divide el arreglo por la mitad antes de fusionar
  ...
  selection_sort: {
    ...
    // Genera la secuencia de estados visuales para el ordenamiento por selección
    generateSteps({ array }) {
      const steps = [];
      const a = [...array];
      const n = a.length;
  ...
  insertion_sort: {
    ...
    // Genera la secuencia de estados visuales para el ordenamiento por inserción
    generateSteps({ array }) {
      const steps = [];
      const a = [...array];
      const n = a.length;
      ...
      for (let i = 1; i < n; i++) {
        ...
        steps.push({ 
          ...
          pivot: i, phase: 'pivot',
          description: `Tomamos a[${i}]=${key} para insertar` 
        });
        ...
        while (j >= 0 && a[j] > key) {
          steps.push({ 
            ...
            pivot: null, phase: 'compare',
            description: `${a[j]} es mayor que ${key}, desplazando a la derecha` 
          });
        ...
        steps.push({ 
          ...
          pivot: null, phase: 'place',
          description: `Insertado ${key} en posición ${j + 1}` 
        });
      }
  ...
  heap_sort: {
    ...
    // Genera la secuencia de estados visuales para el ordenamiento por montículos
    generateSteps({ array }) {
      const steps = [];
      const a = [...array];
      const n = a.length;
      ...
      // Mantiene la propiedad del montículo máximo (Max-Heap) para un subárbol dado
      function heapify(nSize, i) {
        ...
        if (l < nSize) {
          steps.push({
            array: [...a], comparing: [largest, l], sorted: [...sortedArr],
            pivot: null, phase: 'compare',
            description: `Comparando nodo ${largest} con hijo izq ${l}`
          });
        ...
        if (r < nSize) {
          steps.push({
            array: [...a], comparing: [largest, r], sorted: [...sortedArr],
            pivot: null, phase: 'compare',
            description: `Comparando mayor actual ${largest} con hijo der ${r}`
          });
        ...
        if (largest !== i) {
          steps.push({
            array: [...a], comparing: [i, largest], sorted: [...sortedArr],
            pivot: null, phase: 'swap',
            description: `Intercambiando nodo ${i} con hijo mayor ${largest}`
          });
      ...
      for (let i = n - 1; i > 0; i--) {
        steps.push({
          array: [...a], comparing: [0, i], sorted: [...sortedArr],
          pivot: 0, phase: 'swap',
          description: `Moviendo raíz (mayor) ${a[0]} al final (pos ${i})`
        });
        ...
        steps.push({
          array: [...a], comparing: [], sorted: [...sortedArr],
          pivot: null, phase: 'place',
          description: `Elemento ${a[i]} en su posición final`
        });
